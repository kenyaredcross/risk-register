  ...
  <div class="animate-fade-in">
    ...
    <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div class="flex-1 max-w-md">
        <div class="relative">
          <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-medium-gray" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input
            ...
            class="w-full pl-10 pr-4 py-2 border border-light-border rounded-md focus:ring-2 focus:ring-red-primary focus:border-transparent"
          />
        </div>
      </div>
      <button @click="openCreateModal" class="btn btn-primary whitespace-nowrap">
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-light-border">
            <tr v-if="filteredUsers.length === 0">
              <td colspan="6" class="px-4 py-8 text-center text-medium-gray">
                {{ searchQuery ? 'No users found matching your search' : 'No users available' }}
              </td>
            </tr>
            <tr v-for="user in filteredUsers" :key="user.name" class="hover:bg-off-white transition-colors">
...
const filteredUsers = computed(() => {
  ...
  return users.value.filter(user => {
    ...
    const roles = (user.krcs_roles || []).join(' ').toLowerCase()
