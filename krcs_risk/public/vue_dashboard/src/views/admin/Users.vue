<template>
  <div class="animate-fade-in">
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click.self="closeEditModal">
      <div class="bg-white rounded-lg shadow-elegant-xl max-w-lg w-full p-6">
        <h3 class="text-lg font-bold text-charcoal mb-1">Edit KRCS Roles</h3>
        ...
        <div class="flex space-x-3">
          ...
          <button @click="handleUpdateRoles" :disabled="actionLoading" class="btn btn-primary flex-1">
...
const createForm = ref({ email: '', firstName: '', lastName: '', roles: [] })

const showEditModal = ref(false)
const editingUser = ref(null)
const editForm = ref({ roles: [] })
...
const closeCreateModal = () => {
  ...
  createForm.value = { email: '', firstName: '', lastName: '', roles: [] }
  error.value = ''
}
...
const handleUpdateRoles = async () => {
  ...
  actionLoading.value = true
  error.value = ''
  const result = await api.updateUserRoles(editingUser.value.name, editForm.value.roles)
  actionLoading.value = false
  if (result.success) {
    closeEditModal()
    await loadUsers()
  } else {
    error.value = result.message || 'Failed to update roles'
  }
}
